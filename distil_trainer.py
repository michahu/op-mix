...
class MemoryEfficientSyncRefModelCallback(TrainerCallback):
    ...
    def sync_target_model_memory_efficient(model, target_model, alpha):
        ...
        # Build a lookup from target_model so we only sync matching params
        ref_params = dict(target_model.named_parameters())
        ...
        if is_zero3:
            ...
            for name, model_param in model.named_parameters():
                if name not in ref_params:
                    continue
                MemoryEfficientSyncRefModelCallback._sync_param(model_param, ref_params[name], alpha)
    ...
    def _generate_single_turn(self, prompts: list[str], images: Optional[list]):
        ...
        if self.use_vllm:
            if self.vllm_mode == "colocate" and self.args.vllm_enable_sleep_mode:
                # Offload ref_model to CPU to free GPU memory for vLLM wake_up
                # if self.ref_model is not None:
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                if self.args.vllm_enable_sleep_mode:
                    self.llm.sleep(level=1)
                    # Reload ref_model to GPU after vLLM releases memory
                    if self.ref_model is not None:
